package com.rocxie.circleprogresslibrary;
...
public class CircleProgressView extends View {
    ...
    private int mProgressValue = 0;//当前进度
...
//        draHeadText(canvas, cx, cy);
    ...
    private void draText(Canvas canvas){
        if(mStartupText){
//            float textWidth = mScalePaint.measureText(6+"%");
            float x = arcRadius- mScalePaint.measureText(String.valueOf(6+"%")) / 2;
            float y = arcRadius + mTextSize / 5;
            canvas.drawText(mProgressValue+"%",x,y,mScalePaint);
        }
    }
...
//    }
    ...
    private void toProgressValue(final int value, final float beforeValue) {
